...
OTTO has four limit switches: one for each axis (X, Y, and Z) and one for the floating head. The floating head limit switch acts as an obstruction sensor and it should only be triggered when a pipette is loaded. Any other time means that the Z-axis has run into something that it should not have. More information about the floating head is found on the mechanical section. 
